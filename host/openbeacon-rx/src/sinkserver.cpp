...
void *sink_server_handler(void *argv)
{
    int socket_desc , client_sock , c, one;
    int *new_client_sock;
    struct sockaddr_in server , client;

    if (pthread_mutex_init(&g_map_lock, NULL) != 0)
    {
        printf("\nsink_server_handler: system Error - pthread_mutex_init failed.\n");
        return NULL;
    }

    if(argv!=NULL)
        strcpy(logfilename,(char*)argv);

    /*Create socket*/
    socket_desc = socket(AF_INET , SOCK_STREAM , 0);
    if (socket_desc == -1)
    {
        printf("sink_server_handler: Could not create socket");
        return NULL;
    }
    if (quiet_flag==0) printf("sink_server_handler: Socket created.\n");

    /* sockets are typically kept in a kind of limbo for a minute or two
     after finished listening on them to prevent communications intended
     for the previous process coming to new one: the 'TIME_WAIT' state.
     To override that behaviour use setsockopt to set the SO_REUSEADDR
     flag against the socket before listening on it.
    */
    one = 1;
    ...
    if( bind(socket_desc,(struct sockaddr *)&server , sizeof(server)) < 0)
    {
        ...
        perror("sink_server_handler: bind failed. Error");
        return NULL;
    }
    if (quiet_flag==0) printf("sink_server_handler: bind done.\n");

    //Listen
    listen(socket_desc , 30);

    //Accept and incoming connection
    if (quiet_flag==0) printf("sink_server_handler: Waiting for incoming connections...\n");
    ...
    {
        printf("\nsink_server_handler: \n%s", ctime(&mytime));
        printf("\n S: 0-newly connected, 1-reconnected with new ip, 2-still connected.\n");
        printf(" -------------------------+----+---+-------------------+------------------\n");
    ...
    {
        if (quiet_flag==0) printf("sink_server_handler: Connection accepted.\n");
        ...
        *new_client_sock = client_sock;
        ...
        {
            perror("sink_server_handler: could not create thread");
            return NULL;
        }

        //Now join the thread , so that we dont terminate before the thread
        //pthread_join( thread_id , NULL);
        if (quiet_flag==0) {printf("\nsink_server_handler: Handler assigned:"); fprintPt(stdout, thread_id);}
    }
    ...
    pthread_mutex_destroy(&g_map_lock);
...
void *connection_handler(void *socket_desc)
{
	...
	if ( read(sock,buffer,36) != 36 )
		{ if (quiet_flag==0) printf("sink_server_handler: L%d: ERROR reading from socket", __LINE__); return 0;}
	...
	if (write(sock,"7784c4d9-b39f-4f7b-81c9-8b1fc30fa342",36) != 36)
		{if (quiet_flag==0) printf("sink_server_handler: L%d: ERROR writing to socket", __LINE__); return 0;}
