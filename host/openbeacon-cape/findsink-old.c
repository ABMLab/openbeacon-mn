...
int checkdatasink(const char *iface, const int wait_time);
 ...
 */
      ...
      *(cp++)='\0';	/* patcopy points to IPnet, cp points to bits */
      ...
      {err_msg("ERROR: %s is not a valid IP address", patcopy); return;}
 ...
 */
      ...
      *(cp++)='\0';	/* patcopy points to IPnet, cp points to netmask */
      ...
      {err_msg("ERROR: %s is not a valid netmask", patcopy); return;}
 ...
 */
      ...
      *(cp++)='\0';	/* patcopy points to IPstart, cp points to IPend */
      ...
      {err_msg("ERROR: %s is not a valid IP address", cp);return;}
...
struct _tagcheckdatasink_data
{
    int clientSocket;
    struct sockaddr_in serverAddr;
    struct timeval tv;
	unsigned char ipbytes[4];
} g_checkdatasink_data;

int checkdatasink_end(const int ret)
{
		close(g_checkdatasink_data.clientSocket);
        cleanup();
		return ret;
}

int checkdatasink_begin(const char *interface)
{
    memset(&g_checkdatasink_data, 0, sizeof(g_checkdatasink_data));

 	uint32_t ip_addr = 0;
	get_source_ip(interface, &ip_addr);
	g_checkdatasink_data.ipbytes[0] = ip_addr & 0xFF;
	g_checkdatasink_data.ipbytes[1] = (ip_addr >> 8) & 0xFF;
	g_checkdatasink_data.ipbytes[2] = (ip_addr >> 16) & 0xFF;
	g_checkdatasink_data.ipbytes[3] = (ip_addr >> 24) & 0xFF;

	/*---- Create the socket. The three arguments are: ----*/
	/* 1) Internet domain: PF_INET or AF_INET? 2) Stream socket 3) Default protocol (TCP in this case) */
	g_checkdatasink_data.clientSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);

	/*---- Configure settings of the server address struct ----*/
	/* Address family = Internet */
	g_checkdatasink_data.serverAddr.sin_family = AF_INET;
	/* Set port number, using htons function to use proper byte order */
	g_checkdatasink_data.serverAddr.sin_port = htons(7891);
	/* Set IP address to localhost */
	/*g_checkdatasink_data.serverAddr.sin_addr.s_addr = inet_addr(iface);*//*"127.0.0.1");*/
	/* Set all bits of the padding field to 0 */
	memset(g_checkdatasink_data.serverAddr.sin_zero, '\0', sizeof(g_checkdatasink_data.serverAddr.sin_zero));

	g_checkdatasink_data.tv.tv_sec = 1;  /* 1 Sec Timeout */
	g_checkdatasink_data.tv.tv_usec = 0;  /* Not init'ing this can cause strange errors */
	if (setsockopt (g_checkdatasink_data.clientSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&g_checkdatasink_data.tv, sizeof(g_checkdatasink_data.tv)) < 0)
		return checkdatasink_end(1);
	if (setsockopt (g_checkdatasink_data.clientSocket, SOL_SOCKET, SO_SNDTIMEO, (char *)&g_checkdatasink_data.tv, sizeof(g_checkdatasink_data.tv)) < 0)
		return checkdatasink_end(1);
...
int checkdatasink(const char *iface, const int wait_time)
{
	...
	if (quiet_flag==0)
		printf("L%d: Connecting to %s ... ",__LINE__, iface);

	/*---- Connect the socket to the server using the address struct ----*/
	if ( connect(g_checkdatasink_data.clientSocket, (struct sockaddr *) &g_checkdatasink_data.serverAddr, g_addr_size) < 0 )
	{
		if (quiet_flag==0)
			printf("L%d: No Connection or rejected!\n",__LINE__);
		return 0;
	}
	if (quiet_flag==0)
		printf("connected.\n");
...
*/
	if (quiet_flag==0)
		printf("L%d: Sending data to %s ... \n",__LINE__, iface);
	/* Send the word to the server */
	if (send(g_checkdatasink_data.clientSocket, "c67e7c76-1104-4afd-ab97-2b714a1bfd6a", 36, 0) != 36)
		return 0;

    struct timeval tv;
	tv.tv_sec = 3*wait_time;  /* 3 Secs Timeout*/
	tv.tv_usec = 0;  /* Not init'ing this can cause strange errors*/
	setsockopt(g_checkdatasink_data.clientSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(struct timeval));
	if (quiet_flag==0)
		printf("L%d: Listening to %s ... \n",__LINE__, iface);
	if (setsockopt (g_checkdatasink_data.clientSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&g_checkdatasink_data.tv, sizeof(g_checkdatasink_data.tv)) < 0)
		return 0;
	...
	if (recv(g_checkdatasink_data.clientSocket, buffer, 36, 0/*MSG_PEEK*/) != 36)
	{
		if (quiet_flag==0)
			printf("L%d: no response from %s.\n",__LINE__, iface);
		return 0;
	}
	if (memcmp(buffer, "7784c4d9-b39f-4f7b-81c9-8b1fc30fa342",36))
	{
		if (quiet_flag==0)
			printf("L%d: Invalid response from %s.\n",__LINE__, iface);
		return 0;
	}
	/* Send the mac and ip addresses of the reader to the server */
	if (send(g_checkdatasink_data.clientSocket, source_mac, 6, 0) != 6)
		return 0;

	/*	char szip_addr[16];
	sprintf(szip_addr, "%d.%d.%d.%d", ipbytes[0], ipbytes[1], ipbytes[2], ipbytes[3]);
	printf("%s %s\n", interface, szip_addr);*/
	if (send(g_checkdatasink_data.clientSocket, g_checkdatasink_data.ipbytes, 4, 0) != 4)
		return 0;

	/*---- Print the received message ----*/
	if (quiet_flag==0)
		printf("Data Sink found on %s using %s.\n", if_name, iface);
...
int getactivedatasink(char *host)
{
    ...
    checkdatasink_begin(host);

	if (gethostbyname (host)!=NULL)
	{
        for(i=0;i<3;i++)
		if (checkdatasink(host, 5)==1)
		{
			return checkdatasink_end(1);
		}
	}
    ...
    struct stringnode *it;
	for(it=g_sinklist; it!=NULL; it=it->prev)
	{
		if ( checkdatasink(it->str, 2) == 1 )
		{
            memset(host, '\0', 16);
			strcpy(host,it->str);
			cleanup();
